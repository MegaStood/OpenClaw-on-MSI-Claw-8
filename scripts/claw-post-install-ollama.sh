...
# *** OLLAMA VERSION — use this if Ollama adds Intel Vulkan support ***
# Run this ONCE after first boot and WiFi connection
# Usage: chmod +x claw8-post-install-ollama.sh && sudo bash claw8-post-install-ollama.sh
# ============================================================
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}Please run with sudo: sudo bash claw8-post-install-ollama.sh${NC}"
    exit 1
fi
...
if [ "$INSTALL_AI" = "y" ] || [ "$INSTALL_AI" = "Y" ]; then
    ...
    echo -e "${YELLOW}  For GPU acceleration, use claw8-post-install.sh (llama.cpp version).${NC}"
    echo ""
...
if [ "$INSTALL_AI" = "y" ] || [ "$INSTALL_AI" = "Y" ]; then
    ...
    echo -e "${YELLOW}  sudo bash claw8-post-install.sh${NC}"
fi
echo ""
